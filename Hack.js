...
document.querySelector("#contentIframe").src =
  "http://192.168.254.254/ipqostc.htm";
const KV_ORIG_STYLES = [];
$KV("link").forEach((el) => {
...
const addHTML = (() => {
  const KV_DIV_BODY = document.createElement("div");
...
function $KV(KV_SELECTOR_) {
  ...
  if (KV_SELECTOR.length <= 1)
    KV_SELECTOR = document.querySelector(KV_SELECTOR_);
  return KV_SELECTOR;
}
...
function KV_AddToTable(KV_TARGET, KVH_NAME, KVH_IP, KVH_MAC) {
  const KVH_TR = document.createElement("tr");
  const KVH_TD_NAME = document.createElement("td");
  const KVH_TD_IP = document.createElement("td");
  const KVH_TD_MAC = document.createElement("td");
...
const KV_DEVICE_AVAILABLE = [];
const KV_DEVICE = [];

function KV_GET_DEVICES() {
  const KV_FRAME = document.createElement("iframe");
...
function KV_START() {
  const KV_WINDOW = document.getElementById("contentIframe").contentWindow;
  const KV_SAVE_INPUT = KV_WINDOW.document.querySelector('input[name="save"]');
  const KV_CHECK_INPUT = KV_WINDOW.document.querySelectorAll(
    'input[name="removeQ"]'
  );
  const KV_TABLE_TO_REMOVE = KV_WINDOW.document.querySelectorAll(
    ".data_common.data_vertical table"
  )[1];
  let KVTINDEX = 0;
  ...
  if (KV_TABLE_TO_REMOVE.querySelectorAll("tr").length > 0) {
    KV_TABLE_TO_REMOVE.querySelectorAll("tr").forEach((KV_EL) => {
      if (KVTINDEX > 0) {
        const KV_TD_TBL = KV_EL.querySelectorAll("td");
        KV_TD_TBL_srcMac = KV_TD_TBL[3].innerHTML;
        const KV_MAC_ADD_LOWER_CASE = String(KV_TD_TBL_srcMac).toLowerCase();
  ...
  const KV_LIST = document.getElementById("KV_FRAME").contentWindow;
  const KV_TABLE = KV_LIST.document.querySelectorAll("table tbody tr");
  for (let i = 1; i < KV_TABLE.length; i++) {
    const KV_TMAC = KV_TABLE[i].querySelectorAll("td")[2].innerText;
  ...
  KV_DEVICE_AVAILABLE.forEach((KV_EL) => {
    ...
    const KV_MAC_ADD_LOWER_CASE = String(KV_EL.macAdd).toLowerCase();
  ...
  if (KV_DEVICE.length > 0) {
    $KV("#KV_INJECTION #KV_UI_TABLE TR")[1].setAttribute(
      "class",
      "table-highlight"
    );
...
function KV_ADD_() {
  ...
  const CURRENT_DEVICE = KV_DEVICE.shift();
  KV_PERCENT = Math.ceil(
    100 - (KV_DEVICE.length / KV_DEVICE_AVAILABLE.length) * 100
  );
  if (KV_PERCENT < 100) KV_PERCENT += KVrandomInteger(1, 3);
  KV_ProgressTo("#KV_INJECTION", KV_PERCENT);

  const KV_WINDOW = document.getElementById("contentIframe").contentWindow;
  const KV_WAN_INTF = KV_WINDOW.document.querySelector('select[name="intf"]');
  const KV_SRC_MAC = KV_WINDOW.document.querySelector('input[name="srcmac"]');
  const KV_UP_FLOOR = KV_WINDOW.document.querySelector(
    'input[name="uprateFloor"]'
  );
  const KV_DOWN_FLOOR = KV_WINDOW.document.querySelector(
    'input[name="downrateFloor"]'
  );
  const KV_UP_CEIL = KV_WINDOW.document.querySelector(
    'input[name="uprateCeiling"]'
  );
  const KV_DOWN_CEIL = KV_WINDOW.document.querySelector(
    'input[name="downrateCeiling"]'
  );
  const KV_ADD_BTN = KV_WINDOW.document.querySelector('input[name="addRule"]');
  const KV_SPEED = [KV_UP_FLOOR, KV_DOWN_FLOOR, KV_UP_CEIL, KV_DOWN_CEIL];
  ...
  for (const KV_ITEM of KV_SPEED) KV_ITEM.value = 100;
  KV_ADD_BTN.click();
  $KV(
...
function KV_REMOVE_() {
  ...
  KV_PERCENT = 0;
  document
    .querySelector("#contentIframe")
    .setAttribute("onload", "KV_REMOVE_DONE();");
